...
- `npm run rag:readiness:smoke`: 4 files / 33 tests; verifies the Document RAG readiness status keeps deterministic local evidence use available when optional endpoints are unset, treats model-backed RAG as ready only when both local model and embedding requests are ready, blocks remote endpoints, and surfaces Ollama `llama-server` failures without leaking local filesystem paths
- `npm run rag:safety:smoke`: 2 files / 24 tests; verifies prompt-like or command-like text inside parsed documents downgrades evidence quality to review-needed, surfaces a visible warning, keeps local paths sanitized, and carries a do-not-follow-original-instructions rule into both the local model handoff and OpenAI-compatible system prompt
- `npm run hwp:smoke:test`: verifies the private HWP/HWPX smoke harness supports basename-only single-file and directory-batch modes, rejects empty directories, rejects unsupported extensions without printing the source directory path, rejects path-plus-directory ambiguity, and fails closed when neither input is provided
...
- `npm test`: 75 files passed / 2 files skipped; 1150 tests passed / 2 skipped
- `npm run build`
- `cargo test --manifest-path src-tauri/Cargo.toml`
...
2. PARTIAL: Search is local relevance-ranked lexical/provenance/signal retrieval
   ...
   boundary warnings, in-app source-grounded answer drafts,
   ...
   isolation, focused long-chunk excerpts,
