...
2. PARTIAL: Search is local lexical/provenance/signal retrieval with snippets,
   clinician-question drafts, chunk-ranked RAG-context packets, focused long-chunk
   excerpts, status/next-action handoff lines, visit-summary RAG evidence sections,
   CSV `document_rag_evidence` rows, and caregiver HTML RAG evidence sections. It
   is not vector RAG or model-backed generative RAG.
