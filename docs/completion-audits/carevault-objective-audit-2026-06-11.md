...
| Search/RAG-like use of parsed documents | `src/documentKnowledge.ts`, `src/documentFilterActions.ts`, `src/documentParserAudit.ts`, `src/documentRagContext.ts`, README | PARTIAL | Parsed attachment bodies are inserted into saved document text, then used by lexical search, clinical-signal quick-search buttons, patient-facing/domain alias expansion, multi-token document filtering, parser provenance terms, local-path-sanitized source snippets, clinician-question drafts, local RAG-context copy/download packets with profile-based query fallback, query-coverage scoring, source-boundary wording, saved-document status/next-action handoff lines, sanitized query labels, and visit-summary Markdown and caregiver HTML sections with ranked parsed-body evidence chunks for matched saved documents. This is useful local retrieval/context packaging, but it is not vector embedding RAG or a generative RAG pipeline. Do not overclaim it as full RAG. |
...
| Parsed-document use in exports | `src/visitPacket.ts`, `src/caregiverExport.ts`, `src/csvExport.ts`, `src/backupState.ts`, README | PASS | Parser source and clinical-signal summaries now flow into visit-summary Markdown, caregiver HTML, CSV rows/labels, and backup export/import scope summaries; visit-summary Markdown and caregiver HTML also include chunk-ranked document RAG evidence sections with saved-document status and next-action handoff lines. Local attachment paths remain excluded. |
...
1. BLOCKED: Real user/private HWP/HWPX smoke is still missing. A public rhwp
...
2. PARTIAL: Search is local lexical/provenance/signal retrieval with snippets,
   clinician-question drafts, chunk-ranked RAG-context packets, status/next-action
   handoff lines, visit-summary RAG evidence sections, and caregiver HTML RAG
   evidence sections. It is not vector RAG or model-backed generative RAG.
...
retrieval, local chunk-ranked RAG-context packaging, visit-summary and caregiver
HTML evidence sections with status/next-action handoff lines, export/use paths,
cervical-cancer care preparation, and
hypertension/diabetes tracking. The remaining blocker is a real user/private
HWP/HWPX sample smoke, plus the explicit limitation that current retrieval is
not vector or generative RAG.
