...
- `npm run rag:ollama:doctor`: started a temporary localhost Ollama server, reached `/api/version`, listed `llama3.2:1b` and `bge-m3:latest`, then failed minimal direct chat and embedding requests with sanitized HTTP 500 output because Ollama could not start `llama-server`; this proves the blocker is below CareVault's RAG request construction
- `npm run typecheck`
- `git diff --check`
