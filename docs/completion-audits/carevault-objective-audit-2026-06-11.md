...
| Search/RAG-like use of parsed documents | `src/documentKnowledge.ts`, `src/documentFilterActions.ts`, `src/documentParserAudit.ts`, `src/documentRagContext.ts`, README | PARTIAL | Parsed attachment bodies are inserted into saved document text, then used by lexical search, patient-facing/domain alias expansion, multi-token document filtering, parser provenance terms, local-path-sanitized source snippets, clinician-question drafts, local RAG-context copy/download packets with sanitized query labels, and visit-summary Markdown sections with ranked parsed-body evidence chunks for matched saved documents. This is useful local retrieval/context packaging, but it is not vector embedding RAG or a generative RAG pipeline. Do not overclaim it as full RAG. |
