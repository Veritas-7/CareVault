...
- `npm run backup:exports:smoke`: 3 files / 120 tests; verifies restored parsed-document RAG evidence, parsed-document care queue question details, and parsed blood-pressure/HbA1c measurement cues appear in CSV, visit-summary Markdown, and caregiver HTML without local attachment paths
...
- `npm test`: 75 files passed / 2 files skipped; 1155 tests passed / 2 skipped
- `npm run build`
- `cargo test --manifest-path src-tauri/Cargo.toml`
