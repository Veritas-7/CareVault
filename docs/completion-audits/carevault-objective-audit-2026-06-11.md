...
- `npm run hwp:synthetic:smoke`: parses generated HWPX section XML care-note fixtures, verifies multi-section HWPX body XML uses numeric section order, verifies cervical-cancer, hypertension, and diabetes cues become parsed text, ranked RAG evidence chunks, and a source-grounded answer draft, and verifies local paths are stripped
...
- `npm test`: 75 files passed / 2 files skipped; 1148 tests passed / 2 skipped
- `npm run build`
- `cargo test --manifest-path src-tauri/Cargo.toml`
