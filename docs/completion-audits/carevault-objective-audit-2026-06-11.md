...
- `npm run care:queue:smoke`: 2 files / 41 tests; verifies parsed clinical document clues become care-queue question details when manual next action is empty
...
- `npm test`: 74 files passed / 2 files skipped; 1139 tests passed / 2 skipped
- `npm run build`
- `cargo test --manifest-path src-tauri/Cargo.toml`
