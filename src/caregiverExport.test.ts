...
describe("caregiverExport", () => {
  ...
  it("includes parsed lab-result document cues in caregiver HTML without local paths", () => {
    const html = buildCaregiverExportHtml(
      {
        ...state,
        documents: [
    ...
    const queueDetailStart = html.indexOf("HWPX 혈액·신장 기능 검사 서류에서");
    const documentQueueFragment =
      queueDetailStart >= 0
        ? html.slice(queueDetailStart, html.indexOf("</li>", queueDetailStart))
        : "";
    ...
    expect(html).toContain("<h2>진료 준비 큐</h2>");
    expect(html).toContain("서류 · 서류 질문");
    expect(html).toContain("HWPX 혈액·신장 기능 검사");
    expect(html).toContain("HWPX 혈액·신장 기능 검사 서류에서 검사결과 관련 단서");
    expect(html).toContain(
      "문서 측정 단서(원문): WBC 3.2 10^3/uL · ANC 1.1 10^3/uL · PLT 118 10^3/uL · Cr 1.4 mg/dL · eGFR 52 mL/min/1.73m2",
    );
    expect(html).toContain("수치 해석, 반복 측정 시점, 약·식사·치료 영향은 진료팀 기준으로 확인합니다.");
    expect(html).toContain("파싱 원천: HWPX 본문 XML: labs.hwpx");
    expect(html).toContain("[local path]");
    expect(documentQueueFragment).not.toContain("처방");
    expect(documentQueueFragment).not.toContain("치료하세요");
    expect(html).not.toContain("/Users/wj/private");
  ...
  it("includes generated cervical record-check drafts in caregiver care queues", () => {
    ...
    expect(html).toContain("증상 · 자궁경부암 기록 메모");
    expect(html).toContain("발생부위·구조 메모 3/10");
    expect(html).toContain("자궁 상부 2/3");
    expect(html).toContain("하부 1/3");
    expect(html).toContain("질과 연결");
    expect(html).toContain("요관");
    expect(html).toContain("림프관 및 림프절");
    expect(html).toContain(
      '근거: <a href="https://www.cancer.go.kr/lay1/program/S1T211C223/cancer/view.do?cancer_seq=4877&amp;menu_seq=4880" target="_blank" rel="noreferrer">국가암정보센터 자궁경부암 발생부위</a>',
    );
    expect(html).not.toContain("출처: 국가암정보센터 자궁경부암 발생부위");
  });
  ...
  it("includes source-backed cervical-cancer care notes in cancer-care caregiver exports", () => {
    ...
    expect(html).toContain("병리조직 확인 메모");
    ...
    expect(html).toContain(">국가암정보센터 자궁경부암 정의 및 종류</a>");
    expect(html).toContain("병기 설명 확인");
    expect(html).toContain("상피내암");
    expect(html).toContain("암의 분류");
    expect(html).toContain("자궁경부에만 국한");
    expect(html).toContain("질벽 상부 2/3");
    expect(html).toContain("자궁 옆 결합조직");
    expect(html).toContain("질의 하부 1/3");
    expect(html).toContain("요관침윤");
    expect(html).toContain("골반·대동맥주위 림프절");
    expect(html).toContain("방광이나 직장 점막");
    expect(html).toContain("원격전이");
    expect(html).toContain("진단서 병기");
    expect(html).toContain(">국가암정보센터 자궁경부암 진행단계</a>");
    expect(html).toContain("치료 선택 기준");
    ...
    expect(html).toContain("항암 부작용 개인차·효과 오해 확인");
    expect(html).toContain("부작용 유무와 치료 효과");
    expect(html).toContain("전혀 별개의 문제");
    expect(html).toContain("같은 항암제");
    expect(html).toContain("같은 용량");
    expect(html).toContain("환자마다");
    expect(html).toContain("몇 개월 또는 몇 년");
    expect(html).toContain("영구 지속");
    expect(html).toContain("폐·신장·심장·생식기관");
    expect(html).toContain("오심·구토");
    expect(html).toContain("구강 궤양");
    expect(html).toContain("골수기능 저하");
    expect(html).toContain("말초신경병증");
    expect(html).toContain("투여 용량");
    expect(html).toContain("약물 종류 변경");
    expect(html).toContain("중단 여부");
    ...
    expect(html).toContain("재발성 치료 선택 확인");
    expect(html).toContain("골반 내 국소 재발");
    expect(html).toContain("원격 재발");
    expect(html).toContain("재발 부위");
    expect(html).toContain("환자의 상태");
    expect(html).toContain("방사선요법");
    expect(html).toContain("동시항암화학방사선치료");
    expect(html).toContain("골반장기적출술");
    expect(html).toContain("요로전환술");
    expect(html).toContain("장전환술");
    expect(html).toContain("단독 병소");
    expect(html).toContain("외과적 절제술");
    expect(html).toContain("다발성 전이");
    ...
    expect(html).toContain("HPV 접종 지연·추가접종 메모");
    ...
    expect(html).toContain("생활요인 근거 경계 메모");
    expect(html).toContain("연관성은 아직 입증되지");
    expect(html).toContain("일반 암예방수칙");
    expect(html).toContain(">국가암정보센터 국민 암예방 수칙 실천지침 자궁경부암</a>");
    expect(html).toContain(">국가암정보센터 국가암검진 검진주기 및 검진방법</a>");
