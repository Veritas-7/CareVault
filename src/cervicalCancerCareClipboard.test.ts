...
describe("cervicalCancerCareClipboard", () => {
  it("formats a clinic-prep note from existing cervical-care sections and sources", () => {
    ...
    expect(text).toContain("오심·구토·설사");
    expect(text).toContain("흉통이나 호흡곤란");
    expect(text).toContain("배뇨시 쓰리거나 빈뇨");
    ...
    expect(text).toContain("방사선치료");
    expect(text).toContain("초기 치료가 일단락");
    expect(text).toContain("한참 후에 우울증");
    expect(text).toContain("좌절감");
    expect(text).toContain("절망감");
    expect(text).toContain("고립감");
    expect(text).toContain("고독감");
    expect(text).toContain("허무감");
    expect(text).toContain("혼자서 관리해야 한다는 부담감");
    expect(text).toContain("재발이나 전이에 대한 막연한 두려움");
    expect(text).toContain("우울한 기분이나 의욕 상실");
    expect(text).toContain("한 달 이상");
    expect(text).toContain("정신건강의학과 상담");
    expect(text).toContain("전문가의 도움");
    ...
    expect(text).toContain("정신건강의학과에 대한 편견");
    expect(text).toContain("약을 한 번 먹으면 끊지 못한다는 통념");
    expect(text).toContain("진료기록");
    expect(text).toContain("취직이나 보험");
    expect(text).toContain("적절한 서비스를 받지 못");
    expect(text).toContain("가벼운 불면");
    expect(text).toContain("약 없이 상담");
    expect(text).toContain("수면제");
    expect(text).toContain("항우울제");
    expect(text).toContain("항불안제");
    expect(text).toContain("증상 조절 목적");
    expect(text).toContain("대부분 단기간에 끊");
    ...
    expect(text).toContain("정신건강의학과 진료 도움 확인 준비");
    expect(text).toContain("주치의 선생님이 정신건강의학과 진료를 권");
    expect(text).toContain("암환자의 절반 가까이");
    expect(text).toContain("투병 중에 정신건강의학적인 문제");
    expect(text).toContain("불면이나 우울, 불안");
    expect(text).toContain("심하거나 오래 지속");
    expect(text).toContain("정신신경 면역학적 기전");
    expect(text).toContain("면역력");
    expect(text).toContain("심리사회적 서비스");
    expect(text).toContain("필수적인 요소");
    expect(text).toContain("정신적 스트레스");
    expect(text).toContain("삶의 질");
    ...
    expect(text).toContain("심장이 빨리 뛰고");
    expect(text).toContain("가슴이 두근");
    expect(text).toContain("땀");
    expect(text).toContain("몸이 떨리");
    expect(text).toContain("숨이 가쁘거나 답답");
    expect(text).toContain("어지럽거나 쓰러질 것 같은 느낌");
    expect(text).toContain("일상생활에 방해");
    expect(text).toContain("잠들기 어렵");
    expect(text).toContain("집중하지 못하거나 결정을 내리지 못");
    expect(text).toContain("솔직하게 털어놓기");
    expect(text).toContain("주의 전환");
    ...
    expect(text).toContain("암 치료 때문에 수면 습관");
    expect(text).toContain("수면 효율");
    expect(text).toContain("일정한 시각에 일어");
    expect(text).toContain("저녁 카페인");
    expect(text).toContain("잠들기 전 2시간");
    ...
    expect(text).toContain("피로도 감소");
    expect(text).toContain("삶의 질");
    expect(text).toContain("주당 150분 이상의 중강도 신체활동");
    expect(text).toContain("주 2회 이상의 근력운동");
    expect(text).toContain("숨이 약간 차지만");
    expect(text).toContain("옆 사람과 대화가 가능한 정도");
    ...
    expect(text).toContain("다양한 색의 과일, 채소, 전곡류");
    expect(text).toContain("육가공품");
    expect(text).toContain("탄 음식");
    expect(text).toContain("짠 음식");
    expect(text).toContain("하루 한 두 잔의 술");
    ...
    expect(text).toContain("뭔가 좋다는 영양 식품");
    expect(text).toContain("안 드시는 것이 가장 안전");
    expect(text).toContain("기름기가 적은 고단백 식품");
    expect(text).toContain("탄수화물도");
    expect(text).toContain("우유 드셔도 되나요");
    expect(text).toContain("드셔도 됩니다");
    expect(text).toContain("고용량으로 그것만 꾸준히");
    expect(text).toContain("너무 달고 기름진 음식");
    ...
    expect(text).toContain("치료 후 이차암 검진 구분 준비");
    expect(text).toContain("이차암은 원발암");
    expect(text).toContain("새로운 부위");
    expect(text).toContain("암 생존자에게");
    expect(text).toContain("다른 암을 겪지 않은 사람보다");
    expect(text).toContain("10% 에서 20%정도");
    expect(text).toContain("자궁경부암");
    expect(text).toContain("흡연");
    expect(text).toContain("폐암 검진");
    expect(text).toContain("저선량 흉부 CT");
    expect(text).toContain("치료받은 병원");
    expect(text).toContain("다른 부위의 이차암");
    expect(text).toContain("국가 검진");
    expect(text).toContain("암생존자 이차암 검진·흡연력 메모");
    expect(text).toContain("암생존자 예방접종·생백신 시점 확인 준비");
    expect(text).toContain("폐렴, 인플루엔자, 코로나");
    expect(text).toContain("사망과 중증화");
    expect(text).toContain("대부분의 예방접종 백신은 사백신");
    expect(text).toContain("일부 예방접종은 생백신");
    expect(text).toContain("항암치료 한 달 전");
    expect(text).toContain("항암치료 마치고 3개월 후");
    expect(text).toContain("대상포진 백신");
    expect(text).toContain("침습성 폐렴구균 감염률");
    expect(text).toContain("10배 이상");
    expect(text).toContain("13가");
    expect(text).toContain("23가");
    expect(text).toContain("5년 후");
    expect(text).toContain("독감 예방접종");
    expect(text).toContain("매년");
    expect(text).toContain("암생존자 예방접종·생백신 시점 메모");
    expect(text).toContain("직업복귀·근무조정 상담 준비");
    expect(text).toContain("직업복귀");
    expect(text).toContain("긍정적 영향");
    expect(text).toContain("경제 상태");
    expect(text).toContain("삶의 질");
    expect(text).toContain("반드시 직장을 그만두어야 할 필요는 없습니다");
    expect(text).toContain("치료계획");
    expect(text).toContain("몸 상태");
    expect(text).toContain("직업의 종류");
    expect(text).toContain("직장 복귀시기");
    expect(text).toContain("근무시간 조정");
    expect(text).toContain("유연근무제");
    expect(text).toContain("재택근무");
    expect(text).toContain("증빙서류");
    expect(text).toContain("회식");
    expect(text).toContain("음주");
