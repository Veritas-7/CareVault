...
describe("csvExport", () => {
  ...
  it("builds a single spreadsheet-friendly CSV from local records", () => {
    ...
    expect(csv).toContain("HPV 접종 후 이상반응·관찰 확인");
    expect(csv).toContain("접종 부위 통증");
    expect(csv).toContain("부종");
    expect(csv).toContain("발적");
    expect(csv).toContain("오심");
    expect(csv).toContain("메스꺼움");
    expect(csv).toContain("근육통");
    expect(csv).toContain("약 80%");
    expect(csv).toContain("일상 활동을 방해할 정도");
    expect(csv).toContain("약 6%");
    expect(csv).toContain("수일 내 회복");
    expect(csv).toContain("호흡곤란");
    expect(csv).toContain("아나필락시스");
    expect(csv).toContain("실신");
    expect(csv).toContain("앉거나 누운 상태");
