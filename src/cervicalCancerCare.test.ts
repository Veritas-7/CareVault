...
import cervicalCancerCareSource from "./cervicalCancerCare.ts?raw";
import cervicalCancerCareTestSource from "./cervicalCancerCare.test.ts?raw";
import cervicalCancerCareClipboardTestSource from "./cervicalCancerCareClipboard.test.ts?raw";
import cervicalCancerCareMetricTestSource from "./cervicalCancerCareMetric.test.ts?raw";
import {
...
describe("cervicalCancerCare", () => {
  ...
  it("keeps central cervical-care files free of tab indentation", () => {
    const sourceFiles = [
      ["cervicalCancerCare.ts", cervicalCancerCareSource],
      ["cervicalCancerCare.test.ts", cervicalCancerCareTestSource],
      [
        "cervicalCancerCareClipboard.test.ts",
        cervicalCancerCareClipboardTestSource,
      ],
      ["cervicalCancerCareMetric.test.ts", cervicalCancerCareMetricTestSource],
    ];

    const filesWithTabs = sourceFiles.flatMap(([fileName, text]) => {
      const linesWithTabs = text
        .split("\n")
        .flatMap((line, index) => (line.includes("\t") ? [index + 1] : []));

      return linesWithTabs.length > 0
        ? [`${fileName}:${linesWithTabs.join(",")}`]
        : [];
    });

    expect(filesWithTabs).toEqual([]);
  });
  ...
  it("keeps official Korean cervical-cancer sources attached", () => {
    ...
    expect(cervicalCancerCareSources.nccAnatomySite.url).toContain(
      "menu_seq=4880",
    );
    expect(cervicalCancerCareSources.nccLymphedemaCare.url).toContain(
      "S1T429C431",
    );
    expect(cervicalCancerCareSources.kdcaHpv.url).toContain(
      "health.kdca.go.kr",
    );
    expect(cervicalCancerCareSources.kdcaHpvNationalImmunization.url).toContain(
      "nip.kdca.go.kr",
    );
    expect(cervicalCancerCareSources.nccHpvInfection.url).toContain(
      "S1T250C254",
    );
    expect(
      cervicalCancerCareSources.nccChemotherapyUnderstanding.url,
    ).toContain("S1T289C290");
    expect(cervicalCancerCareSources.nccChemotherapySideEffects.url).toContain(
      "S1T289C291",
    );
    expect(cervicalCancerCareSources.nccHpvVaccine.url).toContain(
      "menu_seq=4885",
    );
    expect(cervicalCancerCareSources.nccCervicalPrevention.url).toContain(
      "menu_seq=4885",
    );
    expect(cervicalCancerCareSources.nccCervicalRiskFactors.url).toContain(
      "menu_seq=4884",
    );
    expect(
      cervicalCancerCareSources.nccCervicalPracticeGuideline.url,
    ).toContain("6fb06571");
    expect(cervicalCancerCareSources.nccDefinitionTypes.url).toContain(
      "menu_seq=4881",
    );
    expect(cervicalCancerCareSources.nccTreatmentStatus.url).toContain(
      "menu_seq=4896",
    );
    expect(cervicalCancerCareSources.nccDiagnosisMethods.url).toContain(
      "menu_seq=4889",
    );
    expect(cervicalCancerCareSources.nccStage.url).toContain("menu_seq=4890");
    expect(cervicalCancerCareSources.nccDifferentialDiagnosis.url).toContain(
      "menu_seq=4891",
    );
    expect(cervicalCancerCareSources.nccEarlyScreening.url).toContain(
      "menu_seq=4886",
    );
    expect(cervicalCancerCareSources.nccRelatedStatistics.url).toContain(
      "menu_seq=4882",
    );
    expect(
      cervicalCancerCareSources.nccCancerLifeChildrenCommunication.url,
    ).toContain("S1T327C330");
    expect(
      cervicalCancerCareSources.nccCancerLifePsychologicalStability.url,
    ).toContain("S1T327C329");
    expect(
      cervicalCancerCareSources.nccSurvivorDistressAdaptation.url,
    ).toContain("S1T788C790");
    expect(
      cervicalCancerCareSources.nccMentalHealthStressCancerCause.url,
    ).toContain("article_seq=22076");
    expect(
      cervicalCancerCareSources.nccSurvivorPostTreatmentSlump.url,
    ).toContain("article_seq=22077");
    expect(
      cervicalCancerCareSources.nccMentalHealthInsomniaMedicationConcern.url,
    ).toContain("article_seq=22078");
    expect(
      cervicalCancerCareSources.nccMentalHealthPsychiatryConsultBenefits.url,
    ).toContain("article_seq=22079");
    expect(
      cervicalCancerCareSources.nccSurvivorAnxietyManagement.url,
    ).toContain("S1T788C791");
    expect(cervicalCancerCareSources.nccSurvivorSleepManagement.url).toContain(
      "S1T748C794",
    );
    expect(
      cervicalCancerCareSources.nccSurvivorExerciseManagement.url,
    ).toContain("S1T748C795");
    expect(
      cervicalCancerCareSources.nccSurvivorNutritionLifestyle.url,
    ).toContain("S1T748C796");
    expect(
      cervicalCancerCareSources.nccSurvivorHealthyManagementNutrition.url,
    ).toContain("article_seq=22688");
    expect(cervicalCancerCareSources.nccSurvivorWorkReturn.url).toContain(
      "S1T748C798",
    );
    expect(
      cervicalCancerCareSources.nccComplementaryTherapyConsultation.url,
    ).toContain("S1T365C368");
    expect(cervicalCancerCareSources.nccPainAssessment.url).toContain(
      "S1T378C380",
    );
    expect(cervicalCancerCareSources.nccCancerFatigueCoping.url).toContain(
      "S1T420C421",
    );
    expect(cervicalCancerCareSources.nccDyspneaCause.url).toContain(
      "S1T411C414",
    );
    expect(cervicalCancerCareSources.nccDyspneaConsult.url).toContain(
      "S1T411C415",
    );
    expect(cervicalCancerCareSources.nccCoughCause.url).toContain("S1T410C412");
    expect(cervicalCancerCareSources.nccTreatmentEating.url).toContain(
      "S1T471C472",
    );
    expect(cervicalCancerCareSources.nccNauseaDiet.url).toContain("S1T479C481");
    expect(cervicalCancerCareSources.nccVomitingDiet.url).toContain(
      "S1T479C482",
    );
    expect(cervicalCancerCareSources.nccAppetiteLossDiet.url).toContain(
      "S1T479C480",
    );
    expect(cervicalCancerCareSources.nccMouthPainDiet.url).toContain(
      "S1T479C483",
    );
    expect(cervicalCancerCareSources.nccMucositisCare.url).toContain(
      "S1T390C393",
    );
    expect(cervicalCancerCareSources.nccDryMouthDiet.url).toContain(
      "S1T479C485",
    );
    expect(cervicalCancerCareSources.nccTasteChangeDiet.url).toContain(
      "S1T479C484",
    );
    expect(cervicalCancerCareSources.nccDiarrheaDiet.url).toContain(
      "S1T479C488",
    );
    expect(cervicalCancerCareSources.nccConstipationDiet.url).toContain(
      "S1T479C487",
    );
    expect(cervicalCancerCareSources.nccWeightChangeDiet.url).toContain(
      "S1T479C486",
    );
    expect(cervicalCancerCareSources.nccImmuneLowDiet.url).toContain(
      "S1T479C489",
    );
    expect(cervicalCancerCareSources.nccFatigueDepressionDiet.url).toContain(
      "S1T479C490",
    );
    expect(cervicalCancerCareSources.nccInfectionConsult.url).toContain(
      "S1T435C439",
    );
    expect(cervicalCancerCareSources.nccNeuropathyCare.url).toContain(
      "S1T458C460",
    );
    expect(cervicalCancerCareSources.nccSkinChangeCare.url).toContain(
      "S1T454C456",
    );
    expect(cervicalCancerCareSources.nccAnemiaCare.url).toContain("S1T440C444");
    expect(cervicalCancerCareSources.nccBleedingSymptoms.url).toContain(
      "S1T445C448",
    );
    expect(cervicalCancerCareSources.nccHairLossCare.url).toContain(
      "S1T451C453",
    );
    expect(cervicalCancerCareSources.nccHiccupConsult.url).toContain(
      "S1T466C468",
    );
  });
  ...
  it("keeps every patient-visible cervical-care item linked to a known source", () => {
    expect(
      careItems.every((item) => cervicalCancerCareSources[item.sourceId]),
    ).toBe(true);
    expect(
      cervicalCancerCarePriorityItems
  ...
  it("covers patient-visible warning signs without turning them into treatment orders", () => {
    const text = cervicalCancerCareAlerts
    ...
    expect(
      cervicalCancerCareAlerts.every((item) => item.action.includes("진료팀")),
    ).toBe(true);
  });
  ...
  it("turns cervical-cancer topics into clinician-question drafts", () => {
    ...
    expect(
      cervicalCancerCarePrompts.every((item) => item.question.endsWith("?")),
    ).toBe(true);
    expect(
      buildCervicalCancerCarePromptQuestion(cervicalCancerCarePrompts[1]),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(cervicalCancerCarePrompts[5]),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 치료 후 생활 - https://www.cancer.go.kr/",
    );
    expect(
      buildCervicalCancerCarePromptQuestion(cervicalCancerCarePrompts[6]),
    ).toContain(
    ...
    expect(chemotherapyUnderstandingPrompt.sourceId).toBe(
      "nccChemotherapyUnderstanding",
    );
    expect(chemotherapyUnderstandingPrompt.question).toContain(
      "전신에 퍼져있는 암세포",
    );
    expect(chemotherapyUnderstandingPrompt.question).toContain(
      "암의 치료(완치)",
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(chemotherapyUnderstandingPrompt),
    ).toContain(
    ...
    expect(chemotherapySideEffectsPrompt.sourceId).toBe(
      "nccChemotherapySideEffects",
    );
    expect(chemotherapySideEffectsPrompt.question).toContain(
      "부작용 유무와 치료 효과",
    );
    expect(chemotherapySideEffectsPrompt.question).toContain(
      "전혀 별개의 문제",
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(chemotherapySideEffectsPrompt),
    ).toContain(
    ...
    expect(recurrentTreatmentPrompt.question).toContain(
      "동시항암화학방사선치료",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(recurrentTreatmentPrompt),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 치료방법 - https://www.cancer.go.kr/",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(cervicalCancerCarePrompts[11]),
    ).toContain(
      "출처: 국가암정보센터 방사선치료의 부작용 - https://www.cancer.go.kr/",
    );
    expect(
      buildCervicalCancerCarePromptQuestion(cervicalCancerCarePrompts[12]),
    ).toContain(
    ...
    expect(cervicalCancerCarePrompts[13].question).toContain(
      "의료진에게 바로 연락",
    );
    expect(
      buildCervicalCancerCarePromptQuestion(cervicalCancerCarePrompts[13]),
    ).toContain(
      "출처: 국가암정보센터 림프부종 치료 전후관리 - https://www.cancer.go.kr/",
    );
    expect(
      buildCervicalCancerCarePromptQuestion(cervicalCancerCarePrompts[14]),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 식생활 - https://www.cancer.go.kr/",
    );
    expect(cervicalCancerCarePrompts[14].question).toContain(
      "민간요법·건강보조식품",
    );
    ...
    expect(treatmentEatingPrompt.question).toContain(
      "암을 낫게 해주는 특별한 식품이나 영양소는 없",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(treatmentEatingPrompt),
    ).toContain(
    ...
    expect(nauseaDietPrompt.question).toContain(
      "음식 냄새가 나지 않고 환기가 잘 되는",
    );
    ...
    expect(appetiteLossDietPrompt.question).toContain(
      "충분한 휴식을 취한 아침",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(appetiteLossDietPrompt),
    ).toContain(
    ...
    expect(mouthPainDietPrompt.question).toContain(
      "방사선요법, 화학요법 또는 감염",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(mouthPainDietPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(mucositisCarePrompt),
    ).toContain(
    ...
    expect(dryMouthDietPrompt.question).toContain(
      "침분비를 감소시켜 입안을 마르게",
    );
    expect(dryMouthDietPrompt.question).toContain(
      "음식물을 씹고 삼키는 것이 더욱 어려워",
    );
    ...
    expect(tasteChangeDietPrompt.question).toContain(
      "고기나 생선 등의 고단백식품",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(tasteChangeDietPrompt),
    ).toContain(
    ...
    expect(diarrheaDietPrompt.question).toContain(
      "복부와 골반 부위의 방사선치료",
    );
    expect(diarrheaDietPrompt.question).toContain(
      "감염, 음식에 대한 민감성, 불안 및 스트레스",
    );
    ...
    expect(constipationDietPrompt.question).toContain(
      "수분 및 음식섭취가 불충분",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(constipationDietPrompt),
    ).toContain(
    ...
    expect(weightChangeDietPrompt.question).toContain(
      "체중의 감소를 흔하게 경험",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(weightChangeDietPrompt),
    ).toContain(
    ...
    expect(immuneLowDietPrompt.question).toContain(
      "고기·생선·닭고기 분리 보관",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(immuneLowDietPrompt),
    ).toContain(
    ...
    expect(fatigueDepressionDietPrompt.sourceId).toBe(
      "nccFatigueDepressionDiet",
    );
    expect(fatigueDepressionDietPrompt.question).toContain(
      "치료기간 동안 피로감",
    );
    expect(fatigueDepressionDietPrompt.question).toContain(
      "제대로 먹지 못한 것",
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(fatigueDepressionDietPrompt),
    ).toContain(
    ...
    expect(neuropathyCarePrompt.question).toContain(
      "손, 발을 항상 깨끗이 씻고",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(neuropathyCarePrompt),
    ).toContain(
    ...
    expect(skinChangeCarePrompt.question).toContain(
      "파랗거나 보랏빛 피부 또는 타박상",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(skinChangeCarePrompt),
    ).toContain(
    ...
    expect(anemiaCarePrompt.question).toContain(
      "핏기 없는 입술, 잇몸, 눈꺼풀, 손톱 뿌리, 손바닥",
    );
    ...
    expect(bleedingSymptomsPrompt.question).toContain(
      "핀으로 찌른 것처럼 작고 붉은 발진",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(bleedingSymptomsPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(hiccupConsultPrompt),
    ).toContain(
    ...
    expect(hpvScreeningPrompt.question).toContain(
      "접종 후에도 자궁경부암 선별검사",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(pregnancyBirthPrompt),
    ).toContain(
    ...
    expect(psychologicalStabilityPrompt.sourceId).toBe(
      "nccCancerLifePsychologicalStability",
    );
    expect(psychologicalStabilityPrompt.question).toContain(
      "항암화학요법을 받을 때",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(psychologicalStabilityPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(survivorDistressPrompt),
    ).toContain(
    ...
    expect(stressCancerCausePrompt.sourceId).toBe(
      "nccMentalHealthStressCancerCause",
    );
    expect(stressCancerCausePrompt.question).toContain(
      "스트레스를 많이 받아서 암에 걸린 걸까요",
    );
    expect(stressCancerCausePrompt.question).toContain(
      "누군가를 탓하고 싶은 마음",
    );
    expect(stressCancerCausePrompt.question).toContain(
      "스트레스를 불러일으킨 상황이나 사람을 탓",
    );
    expect(stressCancerCausePrompt.question).toContain("직접적인 발암인자");
    expect(stressCancerCausePrompt.question).toContain(
      "암이 생기게 한 원인이라고 할 수는 없습니다",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(stressCancerCausePrompt),
    ).toContain(
    ...
    expect(survivorSlumpPrompt.question).toContain(
      "혼자서 관리해야 한다는 부담감",
    );
    expect(survivorSlumpPrompt.question).toContain(
      "재발이나 전이에 대한 막연한 두려움",
    ...
    expect(insomniaMedicationConcernPrompt.sourceId).toBe(
      "nccMentalHealthInsomniaMedicationConcern",
    );
    expect(insomniaMedicationConcernPrompt.question).toContain(
      "암 진단 후 불면증",
    );
    expect(insomniaMedicationConcernPrompt.question).toContain(
      "정신건강의학과에 대한 편견",
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(insomniaMedicationConcernPrompt),
    ).toContain(
    ...
    expect(
      Object.values(
        buildCervicalCancerCarePromptQuestionDraft(
          insomniaMedicationConcernPrompt,
          "2026-06-15",
        ),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(psychiatryConsultBenefitsPrompt.sourceId).toBe(
      "nccMentalHealthPsychiatryConsultBenefits",
    );
    expect(psychiatryConsultBenefitsPrompt.question).toContain(
      "주치의 선생님이 정신건강의학과 진료를 권",
    );
    expect(psychiatryConsultBenefitsPrompt.question).toContain(
      "암환자의 절반 가까이",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(psychiatryConsultBenefitsPrompt),
    ).toContain(
    ...
    expect(
      Object.values(
        buildCervicalCancerCarePromptQuestionDraft(
          psychiatryConsultBenefitsPrompt,
          "2026-06-15",
        ),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(survivorAnxietyPrompt.question).toContain(
      "어지럽거나 쓰러질 것 같은 느낌",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(survivorAnxietyPrompt),
    ).toContain(
    ...
    expect(sleepManagementPrompt.question).toContain(
      "암 치료 때문에 수면 습관",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(sleepManagementPrompt),
    ).toContain(
    ...
    expect(exerciseManagementPrompt.sourceId).toBe(
      "nccSurvivorExerciseManagement",
    );
    ...
    expect(exerciseManagementPrompt.question).toContain(
      "주당 150분 이상의 중강도 신체활동",
    );
    expect(exerciseManagementPrompt.question).toContain(
      "주 2회 이상의 근력운동",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(exerciseManagementPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(survivorNutritionPrompt),
    ).toContain(
    ...
    const survivorHealthyManagementNutritionPrompt =
      cervicalCancerCarePrompts.find(
        (item) => item.topic === "치료 후 고용량 영양식품·우유 질문 준비",
      )!;
    expect(survivorHealthyManagementNutritionPrompt.sourceId).toBe(
      "nccSurvivorHealthyManagementNutrition",
    );
    expect(survivorHealthyManagementNutritionPrompt.question).toContain(
      "암생존자 예방접종 및 슬기로운 건강관리",
    );
    expect(survivorHealthyManagementNutritionPrompt.question).toContain(
      "너무 고용량",
    );
    ...
    expect(survivorSecondCancerScreeningPrompt.question).toContain(
      "이차암은 원발암",
    );
    expect(survivorSecondCancerScreeningPrompt.question).toContain(
      "새로운 부위",
    ...
    expect(survivorVaccinationPrompt.question).toContain(
      "폐렴, 인플루엔자, 코로나",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(survivorVaccinationPrompt),
    ).toContain(
      "출처: 국가암정보센터 암생존자 예방접종 및 슬기로운 건강관리 - https://www.cancer.go.kr/lay1/bbs/S1T767C750/G/46/view.do?article_seq=22688&condition=&cpage=3&keyword=&rn=33&rows=12",
    );
    expect(
    ...
    expect(survivorWorkPrompt.question).toContain(
      "반드시 직장을 그만두어야 할 필요는 없습니다",
    );
    ...
    expect(complementaryTherapyPrompt.sourceId).toBe(
      "nccComplementaryTherapyConsultation",
    );
    ...
    expect(complementaryTherapyPrompt.question).toContain(
      "요법가들의 직접적인 설명",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(complementaryTherapyPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(painAssessmentPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(fatigueCopingPrompt),
    ).toContain(
    ...
    expect(dyspneaConsultPrompt.question).toContain(
      "피부가 창백하거나 파랗거나",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(dyspneaConsultPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(treatmentStatusPrompt),
    ).toContain(
    ...
    expect(surgicalComplicationPrompt.question).toContain(
      "수술 직후 급성 합병증",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(surgicalComplicationPrompt),
    ).toContain(
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(acuteRadiationPrompt),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 치료의 부작용 - https://www.cancer.go.kr/lay1/program/S1T211C211/cancer/view.do?cancer_seq=4877&menu_seq=4894",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(radiationVaginalPrompt),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 치료의 부작용 - https://www.cancer.go.kr/lay1/program/S1T211C211/cancer/view.do?cancer_seq=4877&menu_seq=4894",
    );
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(precancerTreatmentPrompt),
    ).toContain(
    ...
    expect(earlyInvasiveTreatmentPrompt.question).toContain(
      "침윤성 자궁경부암",
    );
    expect(earlyInvasiveTreatmentPrompt.question).toContain(
      "환자의 연령과 건강상태",
    ...
    expect(
      buildCervicalCancerCarePromptQuestion(earlyInvasiveTreatmentPrompt),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 치료방법 - https://www.cancer.go.kr/lay1/program/S1T211C223/cancer/view.do?cancer_seq=4877&menu_seq=4893",
    );
  ...
  it("builds the late bowel-bladder prompt into editable question draft fields", () => {
    ...
    const draft = buildCervicalCancerCarePromptQuestionDraft(
      prompt,
      "2026-06-15",
    );
  ...
  it("keeps self-check copy as observation guidance", () => {
    ...
    expect(
      cervicalCancerCareChecks.find(
        (item) => item.label === "림프부종 감염·악화 신호",
      )?.detail,
    ).toContain("피부 붉어짐");
    expect(
      cervicalCancerCareChecks.find(
    ...
    expect(
      cervicalCancerCareChecks.find(
        (item) => item.label === "추적검사 일정·결과",
      )?.detail,
    ).toContain("국가암검진 2년 주기");
    expect(
      cervicalCancerCareChecks.find(
        (item) => item.label === "의심 증상 진단검사 준비",
      )?.detail,
    ...
    expect(
      cervicalCancerCareChecks.find((item) => item.label === "병기 설명 메모")
        ?.detail,
    ...
    expect(earlyScreeningGuide?.detail).toContain(
      "전구 질환인 자궁경부이형성증",
    );
    ...
    expect(relatedStatisticsGuide?.detail).toContain(
      "2026년에 발표된 중앙암등록본부",
    );
    expect(relatedStatisticsGuide?.detail).toContain("2023년");
    expect(relatedStatisticsGuide?.detail).toContain(
      "상피내암을 제외시킨 자궁경부암(C53)",
    );
    ...
    expect(
      cervicalCancerCareChecks.find(
        (item) => item.label === "배뇨·배변·출혈 변화 메모",
      )?.detail,
    ).toContain("혈변·혈뇨");
    expect(cervicalCancerCareChecks.map((item) => item.label)).toContain(
      "장폐색·혈변·혈뇨 연락 메모",
    ...
    expect(treatmentMethodBasisGuide?.detail).toContain(
      "초기 침윤성 자궁경부암",
    );
    ...
    expect(diagnosisMethodPurposeGuide?.detail).not.toContain(
      "검사를 받으세요",
    );
    ...
    expect(
      cervicalCancerCareChecks.find(
        (item) => item.label === "장폐색·혈변·혈뇨 연락 메모",
      )?.detail,
    ...
    expect(
      getCervicalCancerCareSource(cervicalCancerCareChecks[0].sourceId)?.label,
    ).toContain("국가암정보센터");
  });
  ...
  it("formats patient-visible source labels for warning cards", () => {
    ...
    expect(
      formatCervicalCancerCareSourceEvidence("kdcaHpvNationalImmunization"),
    ).toContain(
    ...
    expect(
      formatCervicalCancerCareSourceEvidence("nccSurvivorSleepManagement"),
    ).toContain(
      "국가암정보센터 암생존자 수면관리 - https://www.cancer.go.kr/lay1/S1T748C794/contents.do",
    );
    expect(
      formatCervicalCancerCareSourceEvidence("nccSurvivorDistressAdaptation"),
    ).toContain(
    ...
    expect(
      formatCervicalCancerCareSourceEvidence("nccSurvivorAnxietyManagement"),
    ).toContain(
      "국가암정보센터 암생존자 마음관리 - 내 안의 불안 다스리기 - https://www.cancer.go.kr/lay1/S1T788C791/contents.do",
    );
    expect(
      formatCervicalCancerCareSourceEvidence("nccSurvivorExerciseManagement"),
    ).toContain(
      "국가암정보센터 암생존자 운동 - https://www.cancer.go.kr/lay1/S1T748C795/contents.do",
    );
    expect(
      formatCervicalCancerCareSourceEvidence("nccSurvivorNutritionLifestyle"),
    ).toContain(
      "국가암정보센터 암생존자 영양·식생활 - https://www.cancer.go.kr/lay1/S1T748C796/contents.do",
    );
    expect(
      formatCervicalCancerCareSourceEvidence("nccSurvivorWorkReturn"),
    ).toContain(
  ...
  it("builds context-specific accessible labels for official source links", () => {
    expect(
      buildCervicalCancerCareSourceLinkLabels("nccSymptoms", "비정상 질출혈"),
    ).toEqual({
      ariaLabel:
        "비정상 질출혈 공식 출처 국가암정보센터 자궁경부암 일반적 증상 열기",
      title:
        "비정상 질출혈 공식 출처 국가암정보센터 자궁경부암 일반적 증상 열기",
    ...
    expect(
      ...
      ariaLabel:
        "검진 기준 빠른 확인 공식 출처 국가암정보센터 국가암검진 검진주기 및 검진방법 열기",
      title:
        "검진 기준 빠른 확인 공식 출처 국가암정보센터 국가암검진 검진주기 및 검진방법 열기",
  ...
  it("builds spaced accessible labels for cervical care side-list items", () => {
    expect(
      formatCervicalCancerCareListItemAriaLabel(
        cervicalCancerCareRecoveryGuides[0],
      ),
    ).toBe(
  ...
  it("builds item-specific draft action labels for cervical care buttons", () => {
    expect(
      formatCervicalCancerCareAlertDraftActionLabel(
        cervicalCancerCareAlerts[0],
      ),
    ).toBe("비정상 질출혈 자궁경부암 증상 기록 초안 만들기");
    expect(
      formatCervicalCancerCarePromptDraftActionLabel(
        cervicalCancerCarePrompts[0],
      ),
    ).toBe("자궁경부암 추적 자궁경부암 질문 초안 만들기");
    expect(
      formatCervicalCancerCareItemDraftActionLabel(cervicalCancerCareChecks[0]),
    ).toBe("출혈·분비물 기록 자궁경부암 기록 메모 초안 만들기");
  });

  it("builds symptom drafts from warning cards with source labels and URLs", () => {
    const draft = buildCervicalCancerAlertSymptomDraft(
      cervicalCancerCareAlerts[0],
    );
  ...
  it("builds source-backed record drafts from recovery and prevention memo items", () => {
    const recoveryDraft = buildCervicalCancerCareItemSymptomDraft(
      cervicalCancerCareRecoveryGuides.find(
        (item) => item.label === "성생활 재개·통증 상담",
      )!,
    );
    const preventionDraft = buildCervicalCancerCareItemSymptomDraft(
      cervicalCancerCarePreventionGuides.find(
    ...
    expect(
      Object.values({ ...recoveryDraft, ...preventionDraft }).join(" "),
    ).not.toContain("치료하세요");
  });

  it("formats item-level export evidence with source labels and URLs", () => {
    expect(
      formatCervicalCancerCareAlertEvidence(cervicalCancerCareAlerts[0]),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 일반적 증상 - https://www.cancer.go.kr/",
    );
    expect(
      formatCervicalCancerCareItemEvidence(cervicalCancerCareRecoveryGuides[0]),
    ).toContain(
      "출처: 국가암정보센터 자궁경부암 치료 후 생활 - https://www.cancer.go.kr/",
    );
  });
  ...
  it("keeps recovery timing notes source-backed and clinician-confirmation oriented", () => {
    ...
    const mucositisCleanMoistSentence =
    ...
    const insomniaMedicationConcernGuide =
      cervicalCancerCareRecoveryGuides.find(
        (item) => item.label === "불면증·정신과 약 중독 우려 메모",
      );
    const psychiatryConsultBenefitsGuide =
      cervicalCancerCareRecoveryGuides.find(
    ...
    const survivorHealthyManagementNutritionGuide =
      cervicalCancerCareRecoveryGuides.find(
        (item) => item.label === "암생존자 고용량 영양식품·우유 질문 메모",
      );
    const survivorSecondCancerScreeningGuide =
      cervicalCancerCareRecoveryGuides.find(
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(coneRecoveryGuide!).body,
    ...
    expect(
      formatCervicalCancerCareItemEvidence(radicalRecoveryGuide!),
    ).toContain(radicalHysterectomySentence);
    expect(
      buildCervicalCancerCareItemSymptomDraft(radicalRecoveryGuide!).body,
    ).toContain(radicalHysterectomySentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(radicalRecoveryGuide!),
    ).toContain(radicalHysterectomySentence);
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(radicalRecoveryGuide!),
      ).join(" "),
    ).not.toContain("운전하세요");
    ...
    expect(recurrenceSymptomGuide?.detail).toContain(
    ...
    expect(
      formatCervicalCancerCareItemEvidence(recurrenceSymptomGuide!),
    ).toContain(recurrenceSymptomsSentence);
    expect(
      buildCervicalCancerCareItemSymptomDraft(recurrenceSymptomGuide!).body,
    ).toContain(recurrenceSymptomsSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(recurrenceSymptomGuide!),
    ).toContain(recurrenceSymptomsSentence);
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(recurrenceSymptomGuide!),
      ).join(" "),
    ).not.toContain("진단하세요");
    ...
    expect(cervicalDietGuide?.detail).toContain(
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(cervicalDietGuide!).body,
    ).toContain(cervicalDietTreatmentCautionSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(cervicalDietGuide!),
    ).toContain(cervicalDietBoundarySentence);
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(cervicalDietGuide!),
      ).join(" "),
    ).not.toMatch(/암을 낫게|치료 음식|특효|보조식품 권장/);
    expect(treatmentEatingGuide).toMatchObject({
      label: "치료 중 열량·단백질·영양소 메모",
      sourceId: "nccTreatmentEating",
    });
    expect(treatmentEatingGuide?.detail).toContain(
      "영양 상태는 질병의 이환율과 사망률",
    );
    ...
    expect(treatmentEatingGuide?.detail).toContain(
      treatmentEatingSpecialFoodSentence,
    );
    expect(treatmentEatingGuide?.detail).toContain(
      treatmentEatingNutrientSentence,
    );
    expect(
      formatCervicalCancerCareItemEvidence(treatmentEatingGuide!),
    ).toContain(
      "국가암정보센터 치료 중 일반적인 식생활 - https://www.cancer.go.kr/lay1/S1T471C472/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(treatmentEatingGuide!).body,
    ).toContain(treatmentEatingNutrientSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(treatmentEatingGuide!),
    ).toContain("국가암정보센터 치료 중 일반적인 식생활");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(treatmentEatingGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(nauseaDietGuide!).body,
    ).toContain(nauseaDietMealSettingSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(nauseaDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 메스꺼움");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(nauseaDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(vomitingDietGuide?.detail).toContain(
      "구토증상이 있는 경우 먹거나 마시지 않도록",
    );
    expect(vomitingDietGuide?.detail).toContain(
      vomitingDietClearLiquidSentence,
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(vomitingDietGuide!).body,
    ).toContain(vomitingDietSoftMealSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(vomitingDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 구토");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(vomitingDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(appetiteLossDietGuide?.detail).toContain(
      "먹고 싶을 때, 먹을 수 있을 때",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(appetiteLossDietGuide!),
    ).toContain(
      "국가암정보센터 증상별 식생활 - 식욕부진 - https://www.cancer.go.kr/lay1/S1T479C480/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(appetiteLossDietGuide!).body,
    ).toContain(appetiteLossFluidTimingSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(appetiteLossDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 식욕부진");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(appetiteLossDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(mouthPainDietGuide?.detail).toContain(
      "방사선요법, 화학요법 또는 감염",
    );
    expect(mouthPainDietGuide?.detail).toContain(
      "입안 통증, 잇몸의 손상, 인후염 또는 식도염",
    );
    expect(mouthPainDietGuide?.detail).toContain(
      "항암치료의 부작용 때문인지 치과질환인지",
    );
    ...
    expect(mouthPainDietGuide?.detail).toContain(
      "흰죽, 닭죽, 고기죽, 전복죽, 호박죽, 야채죽, 계란죽",
    );
    expect(mouthPainDietGuide?.detail).toContain(
      "쌀미음, 조미음, 잣미음, 깨미음, 녹두미음",
    );
    ...
    expect(mouthPainDietGuide?.detail).toContain(
      mouthPainIrritatingFoodSentence,
    );
    expect(mouthPainDietGuide?.detail).toContain(
      "오렌지, 포도, 레몬, 토마토주스",
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(mouthPainDietGuide!).body,
    ).toContain(mouthPainTemperatureSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(mouthPainDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 입과 목의 통증");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(mouthPainDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(mucositisCareGuide?.detail).toContain(
      mucositisRinseCandidateSentence,
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(mucositisCareGuide!).body,
    ).toContain("입안 통증");
    expect(
      formatCervicalCancerCareListItemAriaLabel(mucositisCareGuide!),
    ).toContain("국가암정보센터 입안의 염증(구내염) 도움이 되는 방법");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(mucositisCareGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(dryMouthDietGuide?.detail).toContain(
      "육수나 국물 등에 담그거나 적셔서",
    );
    ...
    expect(dryMouthDietGuide?.detail).toContain(
      dryMouthSweetSourCautionSentence,
    );
    expect(dryMouthDietGuide?.detail).toContain(
      "의사선생님이나 치과선생님과 상의",
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(dryMouthDietGuide!).body,
    ).toContain(dryMouthSipStrawSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(dryMouthDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 입안의 건조증");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(dryMouthDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(tasteChangeDietGuide?.detail).toContain(
      "보기가 좋고 냄새도 좋은 식품",
    );
    expect(tasteChangeDietGuide?.detail).toContain(
      tasteChangeAlternativeSentence,
    );
    expect(tasteChangeDietGuide?.detail).toContain(
      tasteChangeSeasoningSentence,
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(tasteChangeDietGuide!),
    ).toContain(
      "국가암정보센터 증상별 식생활 - 입맛의 변화 - https://www.cancer.go.kr/lay1/S1T479C484/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(tasteChangeDietGuide!).body,
    ).toContain(tasteChangeMetallicSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(tasteChangeDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 입맛의 변화");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(tasteChangeDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(diarrheaDietGuide!).body,
    ).toContain(diarrheaFluidElectrolyteSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(diarrheaDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 설사");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(diarrheaDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(constipationDietGuide?.detail).toContain(
      "음식 섭취량이 너무 적지 않도록",
    );
    expect(constipationDietGuide?.detail).toContain(constipationFiberSentence);
    expect(constipationDietGuide?.detail).toContain(
      constipationMovementSentence,
    );
    expect(constipationDietGuide?.detail).toContain(
      constipationClinicianSentence,
    );
    expect(
      formatCervicalCancerCareItemEvidence(constipationDietGuide!),
    ).toContain(
      "국가암정보센터 증상별 식생활 - 변비 - https://www.cancer.go.kr/lay1/S1T479C487/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(constipationDietGuide!).body,
    ).toContain(constipationFiberSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(constipationDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 변비");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(constipationDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(weightChangeDietGuide?.detail).toContain(
      weightLossCalorieProteinSentence,
    );
    expect(weightChangeDietGuide?.detail).toContain(
      "김밥, 초밥, 주먹밥, 볶음밥",
    );
    ...
    expect(weightChangeDietGuide?.detail).toContain(
      "염분 함량이 높은 식품(예: 가공식품, 김치, 젓갈, 장아찌류 등)",
    );
    expect(weightChangeDietGuide?.detail).toContain(
      "열량이 높고 영양가가 없는 식품들(예: 청량 음료, 초콜릿, 사탕, 과자류 등)",
    );
    expect(weightChangeDietGuide?.detail).toContain(
      "과일과 야채 그리고 곡류의 섭취를 증가",
    );
    expect(weightChangeDietGuide?.detail).toContain(
      "지방이 없는 부위의 육류제품과 저지방 우유 및 유제품",
    );
    expect(weightChangeDietGuide?.detail).toContain(
      "끓이고 찌는 형태의 요리방법",
    );
    expect(weightChangeDietGuide?.detail).toContain(
      "버터, 마요네즈, 감미료 등을 추가로 사용하지",
    );
    expect(weightChangeDietGuide?.detail).toContain(
      "가능한 고열량의 간식은 먹지",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(weightChangeDietGuide!),
    ).toContain(
      "국가암정보센터 증상별 식생활 - 체중변화 - https://www.cancer.go.kr/lay1/S1T479C486/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(weightChangeDietGuide!).body,
    ).toContain(weightGainConsultSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(weightChangeDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 체중변화");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(weightChangeDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(immuneLowDietGuide?.detail).toContain(
      "조리에 사용되는 기구, 식기, 수저",
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(immuneLowDietGuide!).body,
    ).toContain("제 WBC/ANC");
    expect(
      formatCervicalCancerCareListItemAriaLabel(immuneLowDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 면역기능의 저하");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(immuneLowDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(fatigueDepressionDietGuide?.detail).toContain(
      "치료기간 동안 피로감",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(fatigueDepressionDietGuide!),
    ).toContain(
      "국가암정보센터 증상별 식생활 - 피로감과 우울 - https://www.cancer.go.kr/lay1/S1T479C490/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(fatigueDepressionDietGuide!).body,
    ).toContain("피로 시작 시점");
    expect(
      formatCervicalCancerCareListItemAriaLabel(fatigueDepressionDietGuide!),
    ).toContain("국가암정보센터 증상별 식생활 - 피로감과 우울");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(fatigueDepressionDietGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(infectionConsultGuide?.detail).toContain(
      "구강내 궤양이나 흰색의 반점",
    );
    expect(infectionConsultGuide?.detail).toContain("체온 측정 시각");
    expect(infectionConsultGuide?.detail).toContain("진료팀에 알려야 하는지");
    expect(
      formatCervicalCancerCareItemEvidence(infectionConsultGuide!),
    ).toContain(
      "국가암정보센터 감염 의료진 상담 기준 - https://www.cancer.go.kr/lay1/S1T435C439/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(infectionConsultGuide!).body,
    ).toContain("발열·오한·감염 상담 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(infectionConsultGuide!).body,
    ).toContain("체온 측정 시각");
    expect(
      formatCervicalCancerCareListItemAriaLabel(infectionConsultGuide!),
    ).toContain("국가암정보센터 감염 의료진 상담 기준");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(infectionConsultGuide!),
      ).join(" "),
    ).not.toMatch(
      /감염으로 진단하세요|항생제를 복용하세요|해열제를 복용하세요|응급실로 가세요|치료하세요|처방하세요|진단하세요|암을 낫게|특효/,
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(neuropathyCareGuide!),
    ).toContain(
      "국가암정보센터 신경계이상 증상 및 주의사항 - https://www.cancer.go.kr/lay1/S1T458C460/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(neuropathyCareGuide!).body,
    ).toContain("손발저림");
    expect(
      formatCervicalCancerCareListItemAriaLabel(neuropathyCareGuide!),
    ).toContain("국가암정보센터 신경계이상 증상 및 주의사항");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(neuropathyCareGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(skinChangeCareGuide?.detail).toContain(
      "가능하면 피부를 공기에 노출",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(skinChangeCareGuide!),
    ).toContain(
      "국가암정보센터 피부변화 증상별 대처방법 - https://www.cancer.go.kr/lay1/S1T454C456/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(skinChangeCareGuide!).body,
    ).toContain("피부 발진");
    expect(
      formatCervicalCancerCareListItemAriaLabel(skinChangeCareGuide!),
    ).toContain("국가암정보센터 피부변화 증상별 대처방법");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(skinChangeCareGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(anemiaCareGuide?.detail).toContain(
      "핏기 없는 입술, 잇몸, 눈꺼풀, 손톱 뿌리, 손바닥",
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(anemiaCareGuide!).body,
    ).toContain("빈혈 관련 증상");
    expect(
      formatCervicalCancerCareListItemAriaLabel(anemiaCareGuide!),
    ).toContain("국가암정보센터 빈혈 관리");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(anemiaCareGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(bleedingSymptomsGuide?.detail).toContain(
      bleedingUrinaryReproductiveSentence,
    );
    expect(
      formatCervicalCancerCareItemEvidence(bleedingSymptomsGuide!),
    ).toContain(
      "국가암정보센터 출혈 증상 - https://www.cancer.go.kr/lay1/S1T445C448/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(bleedingSymptomsGuide!).body,
    ).toContain("출혈 관련 증상");
    expect(
      formatCervicalCancerCareListItemAriaLabel(bleedingSymptomsGuide!),
    ).toContain("국가암정보센터 출혈 증상");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(bleedingSymptomsGuide!),
      ).join(" "),
    ).not.toMatch(
      /출혈로 진단하세요|수혈하세요|지혈제를 복용하세요|응고검사를 지시하세요|항암을 중단하세요|방사선을 중단하세요|치료하세요|처방하세요|진단하세요|암을 낫게|특효/,
    );
    ...
    expect(hairLossCareGuide?.detail).toContain(
      hairLossScalpProtectionSentence,
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(hairLossCareGuide!).body,
    ).toContain("탈모·두피 보호·가발 준비 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(hairLossCareGuide!),
    ).toContain("국가암정보센터 탈모 도움이 되는 방법");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(hairLossCareGuide!),
      ).join(" "),
    ).not.toMatch(
      /탈모를 치료하세요|두피약을 바르세요|선크림을 처방하세요|가발을 처방하세요|항암을 중단하세요|치료하세요|처방하세요|진단하세요|암을 낫게|특효/,
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(hiccupConsultGuide!).body,
    ).toContain("딸꾹질 지속·호흡곤란 상담 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(hiccupConsultGuide!),
    ).toContain("국가암정보센터 딸꾹질 의료진 상담 상황");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(hiccupConsultGuide!),
      ).join(" "),
    ).not.toMatch(
      /딸꾹질을 치료하세요|약을 복용하세요|물을 마시세요|숨을 참으세요|종이봉투|눈을 누르세요|설탕을 먹으세요|레몬을 먹으세요|항암을 중단하세요|치료하세요|처방하세요|진단하세요|암을 낫게|특효/,
    );
    ...
    expect(dyspneaConsultGuide?.detail).toContain(
      dyspneaSputumObservationSentence,
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(dyspneaConsultGuide!),
    ).toContain(
      "국가암정보센터 호흡곤란 도움이 되는 방법 - https://www.cancer.go.kr/lay1/S1T411C415/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(dyspneaConsultGuide!).body,
    ).toContain("호흡곤란·흉통·가래 상담 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(dyspneaConsultGuide!).body,
    ).toContain(dyspneaChestPainSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(dyspneaConsultGuide!),
    ).toContain("국가암정보센터 호흡곤란 도움이 되는 방법");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(dyspneaConsultGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(coughCauseGuide?.detail).toContain(
      "말기 암 환자의 기침은 흉막 삼출",
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(coughCauseGuide!).body,
    ).toContain(severeCoughBurdenSentence);
    expect(
      formatCervicalCancerCareListItemAriaLabel(coughCauseGuide!),
    ).toContain("국가암정보센터 기침 원인");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(coughCauseGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(childFamilyCommunicationGuide?.detail).toContain(
      "혼란을 겪지 않도록",
    );
    expect(childFamilyCommunicationGuide?.detail).toContain(
      "나이에 걸맞은 수준",
    ...
    expect(
      formatCervicalCancerCareItemEvidence(childFamilyCommunicationGuide!),
    ).toContain(
      "국가암정보센터 암환자의 생활 - 자녀에게 알리는 방법 - https://www.cancer.go.kr/lay1/S1T327C330/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(childFamilyCommunicationGuide!)
        .body,
    ).toContain("자녀·가족 설명 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(childFamilyCommunicationGuide!)
        .body,
    ).toContain("네 잘못 때문이 아니고");
    expect(
      formatCervicalCancerCareListItemAriaLabel(childFamilyCommunicationGuide!),
    ).toContain("국가암정보센터 암환자의 생활 - 자녀에게 알리는 방법");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(childFamilyCommunicationGuide!),
      ).join(" "),
    ).not.toMatch(/진단하세요|치료하세요|완치/);
    expect(psychologicalStabilityGuide).toMatchObject({
      label: "정서 안정·전문상담 메모",
      sourceId: "nccCancerLifePsychologicalStability",
    });
    expect(psychologicalStabilityGuide?.detail).toContain(
      "항암화학요법을 받을 때",
    );
    expect(psychologicalStabilityGuide?.detail).toContain(
      "우울해지기 쉽습니다",
    ...
    expect(
      formatCervicalCancerCareItemEvidence(psychologicalStabilityGuide!),
    ).toContain(
      "국가암정보센터 암환자의 생활 - 심리적 안정을 위해 - https://www.cancer.go.kr/lay1/S1T327C329/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(psychologicalStabilityGuide!)
        .body,
    ).toContain("정서 안정·전문상담 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(psychologicalStabilityGuide!)
        .body,
    ).toContain("정신과 전문의 상담 기준");
    expect(
      formatCervicalCancerCareListItemAriaLabel(psychologicalStabilityGuide!),
    ).toContain("국가암정보센터 암환자의 생활 - 심리적 안정을 위해");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(psychologicalStabilityGuide!),
      ).join(" "),
    ).not.toMatch(/진단하세요|치료하세요|상담받으세요|운동하세요/);
    expect(survivorDistressGuide).toMatchObject({
      label: "암생존자 디스트레스 자가평가 메모",
      sourceId: "nccSurvivorDistressAdaptation",
    });
    expect(survivorDistressGuide?.detail).toContain(
      "몸과 마음에 나타나는 모든 괴로움",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(survivorDistressGuide!),
    ).toContain(
      "국가암정보센터 암생존자 마음관리 - 변화된 삶에 적응하기 - https://www.cancer.go.kr/lay1/S1T788C790/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(survivorDistressGuide!).body,
    ).toContain("암생존자 디스트레스 자가평가 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(survivorDistressGuide!),
    ).toContain("국가암정보센터 암생존자 마음관리 - 변화된 삶에 적응하기");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(survivorDistressGuide!),
      ).join(" "),
    ).not.toMatch(
      /디스트레스를 진단하세요|치료하세요|처방하세요|자살하세요|혼자 견디세요|암관리를 중단하세요/,
    );
    ...
    expect(stressCancerCauseGuide?.detail).toContain(
      "스트레스를 많이 받아서 암에 걸린 걸까요",
    );
    expect(stressCancerCauseGuide?.detail).toContain(
      "누군가를 탓하고 싶은 마음",
    ...
    expect(stressCancerCauseGuide?.detail).toContain(
      "원인 단정이나 책임 전가로 쓰지 않습니다",
    );
    expect(
      formatCervicalCancerCareItemEvidence(stressCancerCauseGuide!),
    ).toContain(
      "국가암정보센터 암환자 정신건강 - 스트레스와 암 원인 오해 - https://www.cancer.go.kr/lay1/bbs/S1T668C805/G/54/view.do?article_seq=22076&condition=&cpage=4&keyword=&mode=view&rn=46&rows=12",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(stressCancerCauseGuide!).body,
    ).toContain("스트레스 원인 오해·관리 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(stressCancerCauseGuide!),
    ).toContain("국가암정보센터 암환자 정신건강 - 스트레스와 암 원인 오해");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(stressCancerCauseGuide!),
      ).join(" "),
    ).not.toMatch(
      /스트레스 때문에 암에 걸렸습니다|스트레스가 직접적인 암 원인입니다|누군가를 탓하세요|운동하세요|식생활을 바꾸세요|신앙생활을 하세요|면역력이 회복됩니다|악성세포 성장이 줄어듭니다|치료하세요|처방하세요|진단하세요/,
    );
    ...
    expect(survivorSlumpGuide?.detail).toContain(
      "암 진단 후에 수술과 항암화학요법",
    );
    ...
    ).toContain("암치료 후 슬럼프·우울상담 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(survivorSlumpGuide!),
    ).toContain("국가암정보센터 암환자 정신건강 - 암치료 후 슬럼프");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(survivorSlumpGuide!),
      ).join(" "),
    ).not.toMatch(
      /우울증을 진단하세요|항우울제를 처방하세요|치료하세요|혼자 관리하세요|상담받으세요|괜찮으니 참으세요/,
    );
    ...
    expect(insomniaMedicationConcernGuide?.detail).toContain(
      "암 진단 후 잠을 잘 못 잘 때",
    );
    expect(insomniaMedicationConcernGuide?.detail).toContain(
      "정신건강의학과 약을 먹으면 중독되는지",
    );
    expect(insomniaMedicationConcernGuide?.detail).toContain(
      "정신건강의학과에 대한 편견",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(insomniaMedicationConcernGuide!),
    ).toContain(
      "국가암정보센터 암환자 정신건강 - 불면증과 정신과 약 중독 우려 - https://www.cancer.go.kr/lay1/bbs/S1T668C805/G/54/view.do?article_seq=22078&condition=&cpage=2&keyword=&mode=view&rn=49&rows=12",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(insomniaMedicationConcernGuide!)
        .body,
    ).toContain("불면증·정신과 약 중독 우려 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(
        insomniaMedicationConcernGuide!,
      ),
    ).toContain(
    ...
    expect(psychiatryConsultBenefitsGuide?.detail).toContain(
      "주치의가 정신건강의학과 진료를 권했을 때",
    );
    expect(psychiatryConsultBenefitsGuide?.detail).toContain(
      "암환자의 절반 가까이",
    );
    ...
    expect(psychiatryConsultBenefitsGuide?.detail).toContain(
      "개인 예후 개선으로 단정하지 않습니다",
    );
    expect(
      formatCervicalCancerCareItemEvidence(psychiatryConsultBenefitsGuide!),
    ).toContain(
      "국가암정보센터 암환자 정신건강 - 정신건강의학과 진료 도움 - https://www.cancer.go.kr/lay1/bbs/S1T668C805/G/54/view.do?article_seq=22079&condition=&cpage=2&keyword=&mode=view&rn=48&rows=12",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(psychiatryConsultBenefitsGuide!)
        .body,
    ).toContain("정신건강의학과 진료 도움 확인 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(
        psychiatryConsultBenefitsGuide!,
      ),
    ).toContain("국가암정보센터 암환자 정신건강 - 정신건강의학과 진료 도움");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(
          psychiatryConsultBenefitsGuide!,
    ...
    expect(survivorAnxietyGuide?.detail).toContain(
      "어지럽거나 쓰러질 것 같은 느낌",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(survivorAnxietyGuide!),
    ).toContain(
      "국가암정보센터 암생존자 마음관리 - 내 안의 불안 다스리기 - https://www.cancer.go.kr/lay1/S1T788C791/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(survivorAnxietyGuide!).body,
    ).toContain("암생존자 불안신호·주의전환 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(survivorAnxietyGuide!),
    ).toContain("국가암정보센터 암생존자 마음관리 - 내 안의 불안 다스리기");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(survivorAnxietyGuide!),
      ).join(" "),
    ).not.toMatch(
      /불안을 치료하세요|호흡훈련을 하세요|주의전환을 하세요|약을 복용하세요|진단하세요|처방하세요|치료하세요|괜찮으니 참으세요/,
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(sleepManagementGuide!),
    ).toContain(
      "국가암정보센터 암생존자 수면관리 - https://www.cancer.go.kr/lay1/S1T748C794/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(sleepManagementGuide!).body,
    ).toContain("불면·수면효율·습관 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(sleepManagementGuide!),
    ).toContain("국가암정보센터 암생존자 수면관리");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(sleepManagementGuide!),
      ).join(" "),
    ).not.toMatch(
      /수면제를 복용하세요|약물 치료하세요|인지 행동 치료를 받으세요|치료하세요|처방하세요|진단하세요/,
    );
    ...
    expect(exerciseManagementGuide?.detail).toContain(
      "주당 150분 이상의 중강도 신체활동",
    );
    expect(exerciseManagementGuide?.detail).toContain("주 2회 이상의 근력운동");
    expect(exerciseManagementGuide?.detail).toContain(
      "숨이 약간 차지만 옆 사람과 대화가 가능한 정도",
    );
    expect(exerciseManagementGuide?.detail).toContain(
      "정보교육과 스트레칭, 전신 근력운동",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(exerciseManagementGuide!),
    ).toContain(
      "국가암정보센터 암생존자 운동 - https://www.cancer.go.kr/lay1/S1T748C795/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(exerciseManagementGuide!).body,
    ).toContain("암생존자 운동강도·근력운동 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(exerciseManagementGuide!),
    ).toContain("국가암정보센터 암생존자 운동");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(exerciseManagementGuide!),
      ).join(" "),
    ).not.toMatch(
      /운동하세요|근력운동을 하세요|유산소 운동을 하세요|치료하세요|처방하세요|진단하세요|재발을 막습니다|사망률을 낮춥니다/,
    );
    ...
    expect(survivorNutritionGuide?.detail).toContain(
      "다양한 색의 과일, 채소, 전곡류",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(survivorNutritionGuide!),
    ).toContain(
      "국가암정보센터 암생존자 영양·식생활 - https://www.cancer.go.kr/lay1/S1T748C796/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(survivorNutritionGuide!).body,
    ).toContain("암생존자 균형식·가공육·보조식품 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(survivorNutritionGuide!),
    ).toContain("국가암정보센터 암생존자 영양·식생활");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(survivorNutritionGuide!),
      ).join(" "),
    ).not.toMatch(
      /식단을 처방하세요|육가공품을 절대 먹지 마세요|술을 마시면 안 됩니다|보조식품을 복용하세요|재발을 막습니다|치료하세요|처방하세요|진단하세요/,
    );
    ...
    expect(survivorHealthyManagementNutritionGuide?.detail).toContain(
      "너무 고용량",
    );
    expect(survivorHealthyManagementNutritionGuide?.detail).toContain(
      "뭔가 좋다는 영양 식품",
    ...
    expect(
      formatCervicalCancerCareItemEvidence(
        survivorHealthyManagementNutritionGuide!,
      ),
    ).toContain(
    ...
    ).toContain("암생존자 고용량 영양식품·우유 질문 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(
        survivorHealthyManagementNutritionGuide!,
      ),
    ).toContain("국가암정보센터 암생존자 예방접종 및 슬기로운 건강관리");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(
          survivorHealthyManagementNutritionGuide!,
    ...
    expect(survivorSecondCancerScreeningGuide?.detail).toContain(
      "이차암은 원발암",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(survivorSecondCancerScreeningGuide!),
    ).toContain(
      "국가암정보센터 암생존자 예방접종 및 슬기로운 건강관리 - https://www.cancer.go.kr/lay1/bbs/S1T767C750/G/46/view.do?article_seq=22688&condition=&cpage=3&keyword=&rn=33&rows=12",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(
        survivorSecondCancerScreeningGuide!,
      ).body,
    ).toContain("암생존자 이차암 검진·흡연력 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(
        survivorSecondCancerScreeningGuide!,
      ),
    ...
    expect(survivorVaccinationGuide?.detail).toContain(
      "폐렴, 인플루엔자, 코로나",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(survivorVaccinationGuide!),
    ).toContain(
      "국가암정보센터 암생존자 예방접종 및 슬기로운 건강관리 - https://www.cancer.go.kr/lay1/bbs/S1T767C750/G/46/view.do?article_seq=22688&condition=&cpage=3&keyword=&rn=33&rows=12",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(survivorVaccinationGuide!).body,
    ).toContain("암생존자 예방접종·생백신 시점 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(survivorVaccinationGuide!),
    ).toContain("국가암정보센터 암생존자 예방접종 및 슬기로운 건강관리");
    expect(
      Object.values(
    ...
    expect(survivorWorkGuide?.detail).toContain(
      "반드시 직장을 그만두어야 한다는 뜻은 아니며",
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(survivorWorkGuide!).body,
    ).toContain("암생존자 직업복귀·근무조정 메모");
    expect(
      formatCervicalCancerCareListItemAriaLabel(survivorWorkGuide!),
    ).toContain("국가암정보센터 암생존자 직업복귀");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(survivorWorkGuide!),
      ).join(" "),
    ).not.toMatch(
      /복귀하세요|일하세요|퇴사하지 마세요|그만두세요|치료하세요|처방하세요|진단하세요|술을 마셔도 됩니다|매운 음식을 먹어도 됩니다/,
    );
    ...
    expect(complementaryTherapyGuide?.detail).toContain(
      "요법가들의 직접적인 설명",
    );
    ...
    expect(
      formatCervicalCancerCareItemEvidence(complementaryTherapyGuide!),
    ).toContain(
      "국가암정보센터 보완대체요법 상담 - https://www.cancer.go.kr/lay1/S1T365C368/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(complementaryTherapyGuide!).body,
    ).toContain("보완대체요법·약초 공유 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(complementaryTherapyGuide!).body,
    ).toContain("약초나 영양제");
    expect(
      formatCervicalCancerCareListItemAriaLabel(complementaryTherapyGuide!),
    ).toContain("국가암정보센터 보완대체요법 상담");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(complementaryTherapyGuide!),
      ).join(" "),
    ).not.toMatch(/진단하세요|치료하세요|복용하세요|중단하세요|시술하세요/);
    ...
    expect(
      formatCervicalCancerCareItemEvidence(painAssessmentGuide!),
    ).toContain(
      "국가암정보센터 암성 통증평가 항목 - https://www.cancer.go.kr/lay1/S1T378C380/contents.do",
    );
    expect(
      buildCervicalCancerCareItemSymptomDraft(painAssessmentGuide!).body,
    ).toContain("암성 통증 평가 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(painAssessmentGuide!).body,
    ).toContain("숫자통증등급 0~10");
    expect(
      formatCervicalCancerCareListItemAriaLabel(painAssessmentGuide!),
    ).toContain("국가암정보센터 암성 통증평가 항목");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(painAssessmentGuide!),
      ).join(" "),
    ).not.toMatch(/진단하세요|치료하세요|복용하세요|처방하세요|투약하세요/);
    expect(fatigueCopingGuide).toMatchObject({
      label: "암관련 피로·수면·도움요청 메모",
      sourceId: "nccCancerFatigueCoping",
    });
    expect(fatigueCopingGuide?.detail).toContain(
      "피로의 정도를 반드시 정확하게 평가",
    );
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(fatigueCopingGuide!).body,
    ).toContain("암관련 피로·수면·도움요청 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(fatigueCopingGuide!).body,
    ).toContain("피로를 느낄 때의 상황");
    expect(
      formatCervicalCancerCareListItemAriaLabel(fatigueCopingGuide!),
    ).toContain("국가암정보센터 암관련 피로대처");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(fatigueCopingGuide!),
      ).join(" "),
    ).not.toMatch(/진단하세요|치료하세요|운동하세요|상담받으세요/);
    ...
    expect(
      buildCervicalCancerCareItemSymptomDraft(dyspneaCauseGuide!).body,
    ).toContain("호흡곤란·흉통 변화 메모");
    expect(
      buildCervicalCancerCareItemSymptomDraft(dyspneaCauseGuide!).body,
    ).toContain("쉬고 있거나 움직일 때");
    expect(
      formatCervicalCancerCareListItemAriaLabel(dyspneaCauseGuide!),
    ).toContain("국가암정보센터 호흡곤란 원인");
    expect(
      Object.values(
        buildCervicalCancerCareItemSymptomDraft(dyspneaCauseGuide!),
      ).join(" "),
    ).not.toMatch(
    ...
    expect(
      cervicalCancerCareRecoveryGuides.every((item) => item.sourceId),
    ).toBe(true);
    expect(
  ...
  it("adds prevention and screening notes without confusing them with treatment", () => {
    ...
    const lifestyleEvidenceBoundaryGuide =
      cervicalCancerCarePreventionGuides.find(
        (item) => item.label === "생활요인 근거 경계 메모",
      );
    const practiceDailyPreventionGuide =
      cervicalCancerCarePreventionGuides.find(
    ...
    expect(hpvTypeScopeGuide?.detail).toContain(
      "HPV 6, 11, 16, 18, 31, 33, 45, 52, 58형",
    );
    ...
    expect(hpvMaleRelatedCancerGuide?.detail).toContain(
      "여성의 자궁경부암 예방 효과만큼 높지 않습니다",
    );
    ...
    expect(hpvRelatedDiseaseRangeGuide?.detail).toContain(
      "항문 생식기의 사마귀",
    );
    expect(hpvRelatedDiseaseRangeGuide?.detail).toContain(
      "호흡기에 생기는 유두종 병변",
    ...
    expect(hpvAgeEffectGuide?.detail).toContain(
      "사람유두종바이러스에 노출되기 전",
    );
    ...
    expect(hpvAdverseReactionGuide?.detail).toContain(
      "일상 활동을 방해할 정도",
    );
    ...
    expect(hpvDelayedDoseGuide?.detail).toContain(
      "처음부터 다시 시작하지는 않습니다",
    );
    ...
    expect(hpvNationalProgramGuide?.detail).toContain(
      "고위험 유전형(16형,18형)",
    );
    expect(hpvNationalProgramGuide?.detail).toContain(
      "성경험 전에 접종을 완료",
    ...
    expect(hpvInfectionGuide?.detail).toContain(
      "고위험군 바이러스가 지속 감염",
    );
    expect(hpvInfectionGuide?.detail).toContain("배우자의 성 상대자 수");
    expect(hpvInfectionGuide?.detail).toContain(
      "감염을 비난이나 개인 원인으로 단정하지 말고",
    );
    expect(hpvInfectionGuide?.detail).toContain(
      "백신, 콘돔, 정기검진, HPV 검사 필요성",
    );
    ...
    expect(preventionBundleGuide?.detail).toContain(
      "정기적으로 자궁경부 세포검진",
    );
    ...
    expect(lifestyleEvidenceBoundaryGuide?.detail).toContain(
      "직업적·환경적 유해물질",
    );
    expect(lifestyleEvidenceBoundaryGuide?.detail).toContain(
      "연관성은 아직 입증되지",
    ...
    expect(lifestyleEvidenceBoundaryGuide?.detail).not.toContain(
      "음주가 자궁경부암 위험",
    );
    expect(lifestyleEvidenceBoundaryGuide?.detail).not.toContain(
      "비만이 자궁경부암 위험",
    ...
    expect(practiceDailyPreventionGuide?.detail).toContain(
      "자궁경부암 예방, 일상 생활",
    );
    ...
    expect(
      cervicalCancerCarePreventionGuides.every((item) => item.sourceId),
    ).toBe(true);
  });
  ...
  it("does not treat partial or non-integer age text as screening-eligible", () => {
    for (const age of ["20abc", "20.5", "-20", "0x20"]) {
      expect(
        buildCervicalCancerScreeningSummary({ age, sex: "female" }),
      ).toMatchObject({
        status: "나이 입력 필요",
