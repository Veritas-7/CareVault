...
describe("healthRules", () => {
  ...
  it("recognizes FoodSafetyKorea listeria refrigerated and unpasteurized food guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "리스테리아균은 냉장온도에서도 생존하여 증식할 수 있고, 살균처리하지 아니한 우유, 치즈(특히 소프트치즈), 소시지 및 건조 소시지, 리스테리아 원유, 리스테리아 핫도그, 리스테리아 아이스크림, 리스테리아 살균처리하지 아니한 우유, 리스테리아 치즈, 리스테리아 소프트치즈, 리스테리아 소시지, 리스테리아 건조 소시지, 리스테리아 가금육, 리스테리아 가공 가금육, 리스테리아 비가공 가금육, 리스테리아 비가공 식육, 가공·비가공 가금육, 비가공 식육을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea campylobacter raw-meat and poultry food-safety guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "캠필로박터균은 소, 돼지, 개, 고양이, 닭, 우유, 물이 원인이 될 수 있고 캠필로박터 소, 캠필로박터 돼지, 캠필로박터 닭, 캠필로박터 닭고기, 캠필로박터 우유, 캠필로박터 물, 육류의 생식이나 불충분한 가열, 동물(조류 등)의 분변에 의한 오염, 식육(특히 닭고기)의 생식을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea enterohemorrhagic E. coli cooking and cross-contamination guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "장출혈성 대장균은 대장균 O157:H7이 대표적이고, 심하면 출혈성 대장염, 용혈성요독증후군, 혈전성혈소판 감소증이 나타날 수 있으며, 환자나 보균자의 분변, 보균자가 화장실을 비위생적으로 사용할 때, 하천수와 어패류 등에서 분리 검출되는 오염 경로와 장출혈성 대장균 햄, 장출혈성 대장균 치즈, 장출혈성 대장균 소시지, 장출혈성 대장균 채소샐러드, 장출혈성 대장균 분유, 장출혈성 대장균 두부, 장출혈성 대장균 음료수, 장출혈성 대장균 어패류, 장출혈성 대장균 도시락, 장출혈성 대장균 급식 원인식품을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea vibrio parahaemolyticus seafood and sashimi-tool guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "장염비브리오균은 2~4%의 소금물에서 잘 생육하며 해수온도가 15℃ 이상이 되면 급격히 증식하고, 짧은 쉼표 모양을 나타내며, 어패류, 생선회, 수산식품(게장, 생선회, 오징어무침, 꼬막무침 등) 원인식품과 장염비브리오 어패류, 장염비브리오 생선회, 장염비브리오 수산식품, 장염비브리오 게장, 장염비브리오 오징어무침, 장염비브리오 꼬막무침을 확인하고 어패류의 체표와 내장 및 아가미 등에 부착되는 경로를 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea bacillus cereus room-temperature storage and rice-food guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "바실러스 세레우스균은 설사형 독소(Diarrhetic toxin)와 구토형 독소(Emetic toxin)를 생산하며, 향신료 사용 요리, 육류 및 채소의 스프, 푸딩, 볶음밥, 토양 상재균 오염을 확인, 바실러스 향신료 사용 요리, 바실러스 육류 채소 스프, 바실러스 육류 스프, 바실러스 채소 스프, 바실러스 푸딩, 바실러스 쌀밥, 바실러스 볶음밥을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea clostridium perfringens bulk-storage and reheating guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "클로스트리디움 퍼프린젠스균은 아포의 발아 시 독소를 생성하고 A형과 C형이 사람의 식중독에 관여하며, 돼지고기, 닭고기, 칠면조고기 등으로 조리한 식품 및 그 가공품인 동물성 단백질식품과 미리 가열 조리된 후 실온에서 5시간이상 방치된 식품을 확인, 퍼프린젠스 돼지고기 조리식품, 퍼프린젠스 닭고기 조리식품, 퍼프린젠스 칠면조고기 조리식품, 퍼프린젠스 돼지고기 가공품, 퍼프린젠스 닭고기 가공품, 퍼프린젠스 칠면조고기 가공품, 퍼프린젠스 5시간 실온방치 식품을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea yersinia cold-growth and pork-handling guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "여시니아 엔테로콜리티카균은 0~5℃의 냉장고에서도 발육이 가능한 저온세균이고 진공포장에서도 증식할 수 있는 특성이 있으며, 오물, 오염된 물, 돼지고기, 양고기, 쇠고기, 생우유, 아이스크림 같은 주요 원인식품과 여시니아 오염된 물, 여시니아 돼지고기, 여시니아 양고기, 여시니아 쇠고기, 여시니아 생우유, 여시니아 아이스크림을 확인하고, 동물의 분변과 함께 배출되어 음료수나 식품에 오염되는 경로, 저온보관 상태에서도 균이 증식하는 상황을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea clostridium botulinum toxin and stored-food guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "클로스트리디움 보툴리늄균은 그람양성의 편성혐기성 간균이며 세포 한쪽 끝에 난 원형의 아포를 형성하고, A형, B형, E형, 및 F형균이 사람에게 식중독을 일으키며, 독소는 80℃, 20분과 100℃, 1~2분 가열로 파괴되고, 통조림, 병조림, 레토르트 식품, 식육, 소시지 생선 같은 원인식품과 보툴리늄 통조림, 보툴리늄 병조림, 보툴리늄 레토르트 식품, 보툴리늄 식육, 보툴리늄 소시지, 보툴리늄 생선, 보툴리늄 저장식품, 보툴리늄 기타 저장식품을 확인하며, 환경조건이 혐기적일 때 아포가 발아하여 증식하면서 식중독을 발생시킬 정도의 독소를 생산하는 상황을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
