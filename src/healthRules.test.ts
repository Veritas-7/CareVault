...
describe("healthRules", () => {
  ...
  it("recognizes FoodSafetyKorea norovirus contamination and disinfection guidance", () => {
    ...
    const safePracticeAssessment = assessCancerFood(
      "감염자의 변, 구토물에 접촉하지 않으며, 접촉한 경우에는 충분히 세척하고 소독을 하여야 한다, 조리자는 용변을 본 후나 조리하기 전에 반드시 손을 잘 씻고 소독을 하여야 한다, 과일과 채소는 철저히 씻어야 하며, 굴 등의 어패류는 중심온도 85℃에서 1분 이상 가열하여 먹는다, 질병 발생 후 오염된 표면은 소독제로 철저히 세척, 살균하고 바이러스에 감염된 옷과 이불 등은 즉시 비누를 사용하여 뜨거운 물로 세탁하여야 한다, 노로바이러스 감염자 변 구토물 소독, 노로바이러스 굴 85℃ 1분, 노로바이러스 오염 표면 세척 살균, 노로바이러스 뜨거운 물 세탁",
    );
    const safeTerms = safePracticeAssessment.matches.map((match) => match.term);
    const safeMatchesByTerm = Object.fromEntries(
      safePracticeAssessment.matches.map((match) => [match.term, match]),
    ...
    const riskAssessment = assessCancerFood(
      "노로바이러스는 껍질이 없는(Non-envelop) 바이러스이고 주로 분변-구강 경로(Fecal-oral route)를 통하여 감염되며 사람의 장관 내에서만 증식할 수 있고 연중 발생 가능하며 2차 발병률이 높다, 음식(패류, 샐러드, 과일, 냉장식품, 샌드위치, 상추, 냉장조리 햄, 빙과류)이나 노로바이러스 패류, 노로바이러스 샐러드, 노로바이러스 과일, 노로바이러스 냉장식품, 노로바이러스 샌드위치, 노로바이러스 상추, 노로바이러스 냉장조리 햄, 노로바이러스 빙과류, 노로바이러스 물, 특히 사람의 분변에 오염된 물이나 식품과 노로바이러스 분변-구강 경로, 노로바이러스 사람 장관, 노로바이러스 2차 발병률, 노로바이러스 사람 분변 오염을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
  ...
  it("recognizes FoodSafetyKorea campylobacter raw-meat and poultry food-safety guidance", () => {
    ...
    const riskAssessment = assessCancerFood(
      "캠필로박터균은 소, 돼지, 개, 고양이, 닭, 우유, 물이 원인이 될 수 있고 캠필로박터 소, 캠필로박터 돼지, 캠필로박터 개, 캠필로박터 고양이, 캠필로박터 양, 캠필로박터 닭, 캠필로박터 닭고기, 캠필로박터 우유, 캠필로박터 물, 캠필로박터 하천수, 캠필로박터 호수, 캠필로박터 가금류, 캠필로박터 칠면조, 캠필로박터 가축, 캠필로박터 인간 배설물, 캠필로박터 오염분뇨, 캠필로박터 도살·해체 식육 오염, 육류의 생식이나 불충분한 가열, 동물(조류 등)의 분변에 의한 오염, 식육(특히 닭고기)의 생식을 확인",
    );
    const riskTerms = riskAssessment.matches.map((match) => match.term);
    const riskMatchesByTerm = Object.fromEntries(
      riskAssessment.matches.map((match) => [match.term, match]),
    ...
    const sheepFalsePositiveTerms = assessCancerFood("캠필로박터 양성 결과를 기록").matches.map(
      (match) => match.term,
    );
    ...
    expect(foodSafetyGuide?.detail).toContain("인간의 배설물");
    expect(foodSafetyGuide?.detail).toContain("오염분뇨");
    expect(foodSafetyGuide?.detail).toContain("도살·해체");
    ...
    expect(sheepFalsePositiveTerms).not.toContain("캠필로박터 양");
    expect(JSON.stringify([...safePracticeAssessment.matches, ...riskAssessment.matches])).not.toMatch(
      /치료 음식|완치|암을 낫게/,
    );
  });
  ...
  it("recognizes additional NCC stem-cell-transplant immune-suppression dairy fruit and condiment table context", () => {
    const careTeamGuideText = cancerFoodGuideCategories
      .find((category) => category.id === "care-team")
      ?.items.map((item) => `${item.label} ${item.detail} ${item.examples}`)
      .join(" ");
    const immuneSuppressionGuide = cancerFoodGuideCategories
      ...
      ?.items.find((item) => item.label === "NCC 조혈모세포이식 면역억제 식품표 확인");
    const assessment = assessCancerFood(
      "면역억제 살균처리되지 않은 생우유, 면역억제 아이스크림, 면역억제 유산균발효유, 면역억제 냉동과일, 면역억제 마른과일, 면역억제 핫칠리소스, 면역억제 후추, 면역억제 생된장, 면역억제 허브, 면역억제 캔쥬스, 면역억제 식용유, 면역억제 버터, 면역억제 마가린",
    );
    const terms = assessment.matches.map((match) => match.term);
    const matchesByTerm = Object.fromEntries(
      assessment.matches.map((match) => [match.term, match]),
    );
    const genericAssessment = assessCancerFood("생우유, 아이스크림, 후추, 버터, 캔쥬스");

    expect(immuneSuppressionGuide?.detail).toContain("아이스크림");
    expect(immuneSuppressionGuide?.detail).toContain("식용유");
    expect(immuneSuppressionGuide?.examples).toContain("면역억제 아이스크림");
    expect(immuneSuppressionGuide?.examples).toContain("면역억제 캔쥬스");
    expect(assessment.level).toBe("risk");
    expect(terms).toEqual([
      "면역억제 살균처리되지 않은 생우유",
      "면역억제 아이스크림",
    ...
    expect(careTeamGuideText).toContain("면역억제 살균처리되지 않은 생우유");
    expect(careTeamGuideText).toContain("면역억제 마가린");
    expect(formatFoodMatchEvidence(matchesByTerm["면역억제 캔쥬스"])).toContain(
      "국가암정보센터 조혈모세포이식 면역억제 식품표 - https://www.cancer.go.kr/lay1/S1T295C296/contents.do",
    );
    expect(JSON.stringify(assessment.matches)).not.toMatch(
      /치료 음식|완치|암을 낫게|모든 암환자|자궁경부암 치료/,
  ...
  it("recognizes additional NCC stem-cell-transplant immune-suppression grain and raw-egg dressing table context", () => {
    const careTeamGuideText = cancerFoodGuideCategories
      .find((category) => category.id === "care-team")
      ?.items.map((item) => `${item.label} ${item.detail} ${item.examples}`)
      .join(" ");
    const immuneSuppressionGuide = cancerFoodGuideCategories
      ...
      ?.items.find((item) => item.label === "NCC 조혈모세포이식 면역억제 식품표 확인");
    const assessment = assessCancerFood(
      "면역억제 마른과일이 들어간 빵, 면역억제 건포도 빵, 면역억제 건대추 빵, 면역억제 날계란이 들어간 샐러드 드레싱, 면역억제 익힌 곡류, 면역억제 익힌 빵, 면역억제 익힌 밥, 면역억제 익힌 비스켓",
    );
    const terms = assessment.matches.map((match) => match.term);
    const matchesByTerm = Object.fromEntries(
      assessment.matches.map((match) => [match.term, match]),
    ...
    const genericAssessment = assessCancerFood(
      "건포도 빵, 건대추 빵, 샐러드 드레싱, 익힌 곡류, 빵, 밥, 비스켓",
    );

    expect(immuneSuppressionGuide?.detail).toContain("마른과일(건포도, 건대추)이 들어간 빵");
    expect(immuneSuppressionGuide?.detail).toContain("날계란이 들어간 샐러드 드레싱");
    expect(immuneSuppressionGuide?.detail).toContain("익힌 곡류(빵, 밥, 비스켓 등)");
    expect(immuneSuppressionGuide?.examples).toContain("면역억제 건포도 빵");
    expect(immuneSuppressionGuide?.examples).toContain("면역억제 익힌 비스켓");
    expect(assessment.level).toBe("risk");
    expect(terms).toEqual([
      "면역억제 마른과일이 들어간 빵",
      "면역억제 건포도 빵",
    ...
    expect(careTeamGuideText).toContain("면역억제 날계란이 들어간 샐러드 드레싱");
    expect(careTeamGuideText).toContain("면역억제 익힌 곡류");
    expect(formatFoodMatchEvidence(matchesByTerm["면역억제 익힌 곡류"])).toContain(
      "국가암정보센터 조혈모세포이식 면역억제 식품표 - https://www.cancer.go.kr/lay1/S1T295C296/contents.do",
    );
    expect(JSON.stringify(assessment.matches)).not.toMatch(
      /치료 음식|완치|암을 낫게|모든 암환자|자궁경부암 치료/,
