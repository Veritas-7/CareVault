...
- Sidebar navigation: icon plus Korean label, 42px minimum height. Each section link must expose a destination-specific `aria-label` and hover title such as `대시보드 섹션으로 이동`, not only the visible label. The current click/hash/scroll-backed section uses `aria-current="page"` and a visible active state so users can tell which records area is in focus.
...
- Profile mode toggles: cancer-care, diabetes, and hypertension tracking toggles must expose the current on/off state and the next toggle action in accessible labels/titles, while keeping the visible labels compact.
...
- Recent timeline: show compact summary chips for total records, vital/visit/document/symptom/question/lab type counts, and source-backed records before the mixed timeline list so users can scan record coverage before scrolling.
...
- Korean standards applicability chips and saved-document next-action editors must avoid visible text clipping at 320px, 375px, and 760px.
...
- Runtime QA: cmux workspace `암관리` right in-app browser at `http://127.0.0.1:1431/`.
- Worklog: `working.md`.
